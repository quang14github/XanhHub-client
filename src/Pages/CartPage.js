...
export default function CartPage() {
  ...
  const selected=useSelector(state=>state.cart.cart.filter(e=>e.checkout))
  ...
  return (
    <Layout requiredLogin={true}>
     {lock&&<WatingScreen result={result} isComplete onExit={()=>{setLock(false)
        setToCheckout(false);
      ...
      />
      {toCheckout && <Checkout lock={lock} setLock={setLock} onSubmit={onSubmit} />}
      </div>
    </Layout>
  );
}
...
const WatingScreen=({onExit,result})=>{
  ...
  return <div className={styles.waiting}>
          {result.status?<Complete result={result} onExit={onExit}/>:<OnWait onExit={onExit} canExit={showExit}/>}
  </div>
}
const OnWait=({canExit,onExit})=>{
  return <div className={styles.groupWait}>
